...
namespace Bus_Ticket_Booking_Management_System.Controllers
{
    public class PaymentController : Controller
    {
        ...
        {
            //string decodedfare;
            //byte[] data = Convert.FromBase64String(passengerDetails.fare);
            //decodedfare = Encoding.UTF8.GetString(data).Trim('"');

            OrderModel order = new OrderModel()
            {
                OrderAmount =Convert.ToDecimal(599),
                Currency = "INR",
            ...
            RazorPayOptionsModel razorPayOptions = new RazorPayOptionsModel()
            {
                ...
                ImageLogUrl = "",
                OrderId = orderId,
                ProfileName = passengerDetails.PassengerName,
                ProfileContact = passengerDetails.MobiliNo,
                ProfileEmail = passengerDetails.EmailID,
                Notes = new Dictionary<string, string>()
                {
